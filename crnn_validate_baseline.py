...
from ocr_utils import OCRDataset, CRNN, DEVICE, greedy_decode, cer, wer, idx_to_char
...
with torch.no_grad():
    for images, labels, label_lengths in val_loader:
        ...
        # Split labels for batch
        split_labels = []
        idx = 0
        for length in label_lengths:
            split_labels.append("".join([idx_to_char.get(l.item(), "") for l in labels[idx:idx+length]]))
            idx += length
